...
```{r echo=FALSE, message=FALSE, warning=FALSE, "ggpairs analysis"}
library(GGally)
library(scales)
library(memisc)
...
values in g/L, meaning they add the most mass to the liquid. 
...
of acid is more important than overall acidity, within the rangers we see in 
this dataset.
...
```{r echo=FALSE, "Bivariate_Plots total vs free sulfur dioxide"}
ggplot(data=wine,aes(x=total.sulfur.dioxide-free.sulfur.dioxide,y=free.sulfur.dioxide))+
...
ggplot(data=subset(wine,total.sulfur.dioxide<.2),aes(x=total.sulfur.dioxide,y=sulphates))+
  geom_point(alpha=.5,color=I('goldenrod'))+
...
ggplot(data=wine,aes(x=citric.acid))+
  geom_histogram(binwidth=.025,aes(fill=quality))+
  scale_y_log10()+
  scale_fill_brewer(palette='RdYlGn',guide = guide_legend(title = 'Quality', reverse = T,
