...
p.free<-ggplot(data=wine,aes(x=free.sulfur.dioxide,))+
  geom_histogram(binwidth=.005,aes(color=I('gray'),fill=I('lightgoldenrod')))
p.total<-ggplot(data=wine,aes(x=total.sulfur.dioxide,))+
  geom_histogram(binwidth=.01,aes(color=I('gray'),fill=I('goldenrod')))
p.sulphates<-ggplot(data=wine,aes(x=sulphates,))+
  geom_histogram(binwidth=.05,aes(color=I('gray'),fill=I('darkgoldenrod')))

grid.arrange(p.free,p.total,p.sulphates,ncol=1)
...
It is unclear how much this affects quality, as I've never had a "salty" wine, but
it can potentially manifest as a mineral, metallic, or savory taste, and 
...
features to try and model quality. The features of most interest to me are
features that predominately affect one or more of the five "S" steps of tasting. 
These include alcohol, sugar, acids,
